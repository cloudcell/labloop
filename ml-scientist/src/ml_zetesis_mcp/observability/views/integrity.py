...
_CHECK_HELP: list[tuple[str, str]] = [
    (
        ...
        "<code>evidence</code> (loop0-read — the read-only channel "
        "into ml-episteme), <code>promotion</code> (loop0-write — "
        "the separate write path into Loop-0 lineage surfaces), and "
    ...
    (
        ...
        "An open investigation with no evidence pull in "
        "<code>stale_investigation_seconds</code> (default 3600). "
        "Visible debt: the inquiry was opened and abandoned without "
        "a verdict.",
    ),
    (
        ...
        "memory but no mint happened. The two must move together.",
    ),
    (
        "concluded_unminted",
        "An investigation concluded with verdict "
        "<code>findings</code> that still has provisional findings — "
        "the conclusion didn't distill into claims. Whatever the "
        "reason, memory never got the result.",
    ),
    (
        "closed_campaigns_scored",
        "A closed campaign whose <code>promotion_score</code> is "
        "null — the score is frozen at close, so a null means the "
    ...
    (
        ...
        "More than one roster row marked "
        "<code>derived_status = 'champion'</code> — the champion "
        "pointer must be unique; two champions means a promotion "
        "raced or was hand-edited.",
    ),
    (
        "minted_claims_resolve",
        "Every <code>findings.claim_id</code> is fetched through the "
        "claims channel — a minted id that fails to resolve means "
        "the claim is gone or the channel is lying. "
