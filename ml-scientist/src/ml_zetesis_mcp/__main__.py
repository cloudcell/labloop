"""Entry point for the zetesis MCP server (Loop 1 — search loop).

Run via stdio (for local MCP clients):

    uv run ml-zetesis-mcp

Or via Streamable HTTP (for remote/hosted):

    uv run ml-zetesis-mcp --transport http --port 38070

Own port (38070), own store (~/.ml-zetesis/search.db). ADR-0001: one
MCP server per loop. Upstream adaptors ([adaptors.evidence],
[adaptors.claims] in ml-zetesis.toml) connect on the server event loop —
a failed connect disables that channel; the server still runs.
"""
...
def _check_port_available(host: str, port: int, label: str) -> bool:
    try:
        ...
        print(
            ...
            f"  → Another ml-zetesis-mcp process is likely still running.\n"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="ml-zetesis-mcp — Loop 1 search loop (researcher)",
    )
    parser.add_argument(
        "--transport",
        choices=["stdio", "http"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Port for HTTP transport (default: config or 38070)",
    )
    parser.add_argument(
        "--host",
        default=None,
    ...
    parser.add_argument(
        ...
        "(default: ~/.ml-zetesis/search.db)",
    )
    parser.add_argument(
        "--config",
        default=None,
        help="Path to ml-zetesis.toml",
    )
    parser.add_argument(
        "--stateless",
        action="store_true",
    ...
    async def _run_server(coro):
        ...
        """
        labels = {
            "evidence": "evidence (Loop-0)",
            "promotion": "promotion (Loop-0 write)",
    ...
    if transport == "stdio":
        ...
        if not _check_port_available(host, port, "zetesis HTTP server"):
            sys.exit(1)
