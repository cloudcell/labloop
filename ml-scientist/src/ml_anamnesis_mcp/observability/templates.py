"""HTML templates for the anamnesis observability GUI.

Same visual language as ml-episteme's GUI — deliberately duplicated
(ADR-0002): the servers share no code. Keep both copies small.
"""
...
# Peer GUI bases for claim-edge hyperlinks — server key → base URL,
# resolved the same way ([observability] <server>_gui_url or the
# ports.env-derived ML_<SERVER>_GUI_URL). Refs whose owning GUI is
# unconfigured render unlinked rather than guessing an address.
...
def configure_polling(
    ...
    if peer_gui_urls:
        PEER_GUI_URLS.update(
            {k: v.rstrip("/") for k, v in peer_gui_urls.items() if v}
        )
...
<head>
    ...
    <title>{escape(title)} — ml-anamnesis</title>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .status-empirical {{ background: rgba(45,212,191,0.2); color: var(--accent); }}
        .status-methodological {{ background: rgba(139,92,246,0.2); color: #a78bfa; }}
        ...
        .rel-evidence {{ background: rgba(34,197,94,0.15); color: var(--status-active); }}
        .rel-contradicts {{ background: rgba(239,68,68,0.15); color: var(--status-failed); }}
...
<body>
    ...
    {body}
    <script>
        // Same contract as ml-episteme's GUI: a failed health poll
        // means the service is down — flip the pill red instead of
        // leaving a stale green. The replacement keeps polling.
