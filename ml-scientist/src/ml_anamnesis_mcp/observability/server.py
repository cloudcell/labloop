"""Observability web GUI for anamnesis — read-only views over the
claim graph. Same contract as ml-episteme's GUI: a separate port, a
...
from .views import claims as claim_views
...
def create_observability_app(
    ...
    """Create the anamnesis observability Starlette app.

    Args:
        store: The shared MemoryStore (read-only access).
    ...
    def index(request: Request) -> HTMLResponse:
        view = request.query_params.get("view", "live")
        claim_type = request.query_params.get("type")
        return claim_views.render_claim_list(store, view, claim_type)

    def search(request: Request) -> HTMLResponse:
        q = request.query_params.get("q", "")
        return claim_views.render_search_results(store, q)
